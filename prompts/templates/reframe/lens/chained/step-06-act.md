---
name: 'Step 6 - Act'
description: 'Crystallize everything into ONE first action — hand the user a summary card they can carry'
---
...
**Write the file to:** `.codemachine/reframe/session-{YYYY-MM-DD}.xml` (in the current project directory)

If multiple sessions happen on the same day, append a counter: `session-{YYYY-MM-DD}-2.xml`

**Tell the user the file was saved:** "I've saved your session data to `.codemachine/reframe/session-{date}.xml` so we can pick up where we left off anytime."
...
│  press `ENTER`. I'll walk you through the **rest of     │
...
**THEN STOP COMPLETELY. Do not ask questions. Do not add more advice. Do not wait for feedback. Wait for the system to inject the next step.**
